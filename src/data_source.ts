...
export async function get_morning(ctx: Context, config: Config, uid: string, gid: string) {
    ...
    const randomIndex = Math.floor(Math.random() * config.morningPrompts.length);
    return `你是第 ${morningResult.global_rank} 个起床哒，群里第 ${morningResult.num} 个\n睡了 ${morningResult.duration}，${config.morningPrompts[randomIndex]}`;
}
...
async function get_or_create_group(ctx: Context, gid: string) {
    ...
    if (result.length == 0) {
        return await ctx.database.create("sleep_group", { group_id: gid, morning_count: 0, night_count: 0 });
    }
    return result[0];
}
